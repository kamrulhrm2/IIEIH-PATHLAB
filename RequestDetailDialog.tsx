...
import { Check, ChevronDown, ChevronUp, Info, Loader2, Printer } from 'lucide-react';
...
export function RequestDetailDialog({ request, onClose }: RequestDetailDialogProps) {
  ...
  const quotaColor =
    usage >= 5 ? 'bg-red-500' : usage >= 4 ? 'bg-amber-500' : 'bg-emerald-500';
        ...
        <DialogContent className="max-w-[900px]">
          ...
          {canHrAct && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <Progress
                value={Math.min((usage / 5) * 100, 100)}
              ...
              <TableBody>
                {isLoading &&
                  Array.from({ length: 3 }).map((_, i) => (
                    <TableRow key={i}>
                      <TableCell colSpan={showCheckboxes ? 6 : 5}>
