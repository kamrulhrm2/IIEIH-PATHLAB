...
export default function DashboardPage() {
  ...
  const remaining = Math.max(5 - myUsage, 0);
  ...
  const kpis: Kpi[] = useMemo(() => {
    ...
    switch (role) {
      ...
      case 'doctor':
        return [
          { label: 'Doctor Queue', value: count((r) => r.status === 'PENDING_DOCTOR'), icon: Stethoscope, accent: 'bg-emerald-100 text-emerald-700' },
  ...
  const quotaBarColor =
    remaining >= 3 ? 'bg-emerald-500' : remaining >= 1 ? 'bg-amber-500' : 'bg-red-500';
          ...
          kpi.label === 'Remaining Quota' && showQuotaCard ? (
            <Card key={kpi.label} className="transition-colors hover:border-slate-300">
              <CardContent className="p-5">
                ...
                <p className="mt-2 text-xs text-slate-500">{remaining} of 5 remaining</p>
                <Progress
                  value={(remaining / 5) * 100}
                  className="mt-1.5"
